...
export function colorTemp(temperature, units = "metric") {
  const celsius = units === "imperial" ? (temperature - 32) * (5 / 9) : temperature;
  if (celsius <= -10) return chalk.blue(`${temperature}°`);
  if (celsius < 0) return chalk.cyan(`${temperature}°`);
  if (celsius < 10) return chalk.green(`${temperature}°`);
  if (celsius < 20) return chalk.yellow(`${temperature}°`);
  if (celsius < 30) return chalk.magenta(`${temperature}°`);
  return chalk.red(`${temperature}°`);
}
...
function windLabel(speed, units) {
  return units === "imperial" ? `${speed} mph` : `${speed} km/h`;
}

function precipitationLabel(amount, units) {
  return `${fmt(amount)} ${units === "imperial" ? "in" : "mm"}`;
}

function tempLabelColored(temp, units) {
  return `${colorTemp(temp, units)}${units === "imperial" ? "F" : "C"}`;
}
...
export function renderCurrent(obj) {
  ...
  const tempLine = obj.apparent_temperature != null
    ? `${tempLabelColored(obj.temperature, obj.units)} (feels like ${tempLabel(obj.apparent_temperature, obj.units)})`
    : tempLabelColored(obj.temperature, obj.units);
  const lines = [
    title,
    tempLine,
  ...
  if (obj.precipitation_mm != null) {
    lines.push(`Precipitation ${precipitationLabel(obj.precipitation_mm, obj.units)}`);
  }
...
/** Compact one-liner for use inside a conversation (no box). */
export function renderCurrentCompact(obj) {
  if (obj.error) return chalk.dim(`⚠️  ${obj.error}`);
  const feels = obj.apparent_temperature != null
    ? ` (feels ${tempLabel(obj.apparent_temperature, obj.units)})`
    : "";
  const parts = [
    `📍 ${obj.city}`,
    `${wmoIcon(obj.weather_code, obj.is_day)} ${tempLabel(obj.temperature, obj.units)}${feels}`,
  ];
  if (obj.humidity_pct != null) parts.push(`💧 ${obj.humidity_pct}%`);
  if (obj.wind_kmh != null) parts.push(`💨 ${windLabel(obj.wind_kmh, obj.units)}`);
  if (obj.uv_index != null) parts.push(`☀️ UV ${obj.uv_index}`);
...
export function renderForecast(obj) {
  ...
  for (const d of obj.forecast) {
    ...
    const range = `H ${tempLabelColored(d.high, obj.units)} · L ${tempLabelColored(d.low, obj.units)}`;
...
/** Compact one-liner for use inside a conversation (no box). */
export function renderForecastCompact(obj) {
  if (obj.error) return chalk.dim(`⚠️  ${obj.error}`);
  const days = obj.forecast
    .map((d) => `${fmtDate(d.date)} ${wmoIcon(d.weather_code, 1)} ${tempLabel(d.high, obj.units)}/${tempLabel(d.low, obj.units)}`)
    .join("  ");
  return chalk.dim(`📍 ${obj.city} — ${obj.forecast.length}-day · ${days}`);
}
