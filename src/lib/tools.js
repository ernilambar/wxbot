...
async function fetchJson (url) {
  ...
  try {
    ...
    throw new Error(`Weather service request failed: ${err.message}`)
  }

  if (!res.ok) {
    throw new Error(`Weather service request failed (HTTP ${res.status}).`)
  }
...
function validateCity (city) {
  if (typeof city !== 'string' || city.trim() === '') {
    throw new Error('A non-empty city name is required.')
  }
  return city.trim()
}

function normalizeDays (days) {
  return Math.max(1, Math.min(Number.isInteger(days) ? days : 3, 7))
}
...
async function getCurrentWeatherRaw ({ city, units = 'metric' }) {
  ...
  if (!c) throw new Error('Weather service did not return current conditions.')
...
async function getForecastRaw ({ city, days = 3, units = 'metric' }) {
  ...
  url.searchParams.set('longitude', coords.lon)
  url.searchParams.set('timezone', coords.tz)
  url.searchParams.set('forecast_days', String(normalizeDays(days)))
  url.searchParams.set(
    'daily',
  ...
  if (!d?.time || !Array.isArray(d.time)) {
    throw new Error('Weather service did not return forecast data.')
  }
  ...
  return { city: coords.city, units, forecast }
}
